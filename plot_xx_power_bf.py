...
def main ():
    ...
    #5.6023011022100295 3.723913000073144 8.167367656209182 $10^{6}\epsilon_{f}$
    #0.07896675765832575 0.013512194117960993 0.009449299852320833 $f_{*}$
    #0.3346720034337849 0.30517432490247126 2.536744785953383 $C_0$
    #[1.00092913 1.50025228 3.35786378 7.11121924 9.51706093] [-2.35693464 -1.8576115   0.          3.75335546  6.15919716] $10^{6}\epsilon_{f}$
    #[0.06636728 0.07522712 0.08397865 0.09052908 0.09711143] [-0.01761138 -0.00875153  0.          0.00655042  0.01313277] $f_{*}$
    #[0.01002    0.02390788 0.16131257 1.28186051 4.0201153 ] [-0.15129258 -0.13740469  0.          1.12054794  3.85880272] $C_0-1$
    theta_best = [3.359, 3.e-5 ,0.084,0.120000,1.000000,0.45200,0.841000,1.628000,0.10000,1.720000,0.195000,0.010000,0.800000,0.1613,1.0,6.0,3.0]
    theta_cdn = [3.359, 3.e-5 ,0.084,0.120000,1.000000,0.452000,0.841000,1.628000,0.10000,1.720000,0.195000,0.010000,0.800000,0.01,1.0,6.0,3.0]
    theta_cup = [3.359, 3.e-5 ,0.084,0.120000,1.000000,0.452000,0.841000,1.628000,0.10000,1.720000,0.195000,0.010000,0.800000,4.02,1.0,6.0,3.0]

    param_ind_dict = {'eps_f':0, 'eps_DM':1, 'f_star':2, 'S_star':3, 'A_C':4, 'A_nt':5, 'B_nt':6, 'gamma_nt':7, 'gamma_mod0':8, 'gamma_mod_zslope':9, 'x_break':10, 'x_smooth':11, 'n_nt_mod':12, 'clump0':13, 'alpha_clump':14, 'beta_clump':15, 'gamma_clump':16}
    ...
    with open('best_fit.txt','w') as outf:
        print('# ell C_ell C_ell_up C_ell_dn [ergs^2/s^2/cm^4/sr^2]', file=outf)
        for i in np.arange(len(ell)) :
            print(ell[i], cl[i], cl_up[i], cl_dn[i], file=outf)
