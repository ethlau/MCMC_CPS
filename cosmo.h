...
class cosmo {
 ...
 protected:
  double H0, Omega_M, Omega_b, rho_crit, rho_mean, a, Omega_k, Omega_L, wt, hubble;
 ...
 public:
  ...
  double scale_fact(double redshift) {
    a = 1.0/(1.0+redshift);
    return a;
  }

  double hubblez(double redshift) {
    scale_fact(redshift);
    return H0 * Efact(redshift);
  }

  double calc_rho_crit(double redshift) {
    double Hz = hubblez(redshift);
    rho_crit = pow(Hz,2)*3./(8.*PI*gnewt);
    return rho_crit;
  }

  double calc_rho_mean(double redshift) {
    double Hz = hubblez(redshift);
    rho_crit = pow(Hz,2)*3./(8.*PI*gnewt);
    rho_mean = rho_crit*Omega_Mz(redshift);
    return rho_mean;
  }


  double Omega_Mz(double redshift) {
    scale_fact(redshift);
    double Hz = hubblez(redshift);
  ...
  double proper_dist(double redshift) {
    return ang_diam(redshift) * (1.0+redshift);
  }

  double lum_dist(double redshift) {
  ...
  double cosmic_time(double redshift) {
    ...
    double x[16] = {0.0950,0.2816,0.4580,0.6179,0.7554,0.8656,0.9446,0.9894,-0.0950,-0.2816,-0.4580,-0.6179,-0.7554,-0.8656,-0.9446,-0.9894};
    double w[16] = {0.1895,0.1826,0.1692,0.1496,0.1246,0.0952,0.0623,0.0272,0.1895,0.1826,0.1692,0.1496,0.1246,0.0952,0.0623,0.0272};
   
    double a0 = 0.0, a1, Delta_a, h2, sum, cosm_h0, den0 = Omega_M, den1 = Omega_L, den2;
    double ss, ak, adot, cosm_t;
    double time_units, secperyr = 3.15569260e7, age_of_univ  = 8.93120195E+17/2.0; // in seconds
    int Nsteps, i, k;
    Nsteps = 10000;
    a1 = 1.0/(1.0+redshift);
  ...
  double dynrho(double a, double wdyn) {
